...
"""Monitor a vehicle spawned by carla_sensor_kit_utils vehicle_spawner."""

"""
Welcome to CARLA vehicle monitor.

This tool attaches to an existing vehicle spawned by the ROS2 vehicle_spawner.

Use ARROWS or WASD keys for control (when manual control is enabled).
...
class World(object):
    ...
    def restart(self):
        ...
        # Find existing vehicle with matching role name instead of spawning new one
        if self.player is not None:
            # If we already have a player, just destroy sensors to recreate them
            self.destroy_sensors()
        else:
            # Find the vehicle spawned by vehicle_spawner
            ...
            if found_vehicle is None:
                print(
                    f"Error: No vehicle found with role_name '{self.actor_role_name}'"
                )
                print("Available vehicles:")
                for actor in actors:
                    role = actor.attributes.get("role_name", "none")
                    print(f"  {actor.type_id} - role_name: {role}")
                print("Make sure to spawn a vehicle")
                sys.exit(1)

            self.player = found_vehicle
            print(
                f"Attached to existing vehicle: {get_actor_display_name(self.player)}"
            )

            # Get vehicle's max speed from blueprint if available
            blueprint_library = self.world.get_blueprint_library()
            blueprint = blueprint_library.find(self.player.type_id)
        ...
        # Find existing sensors attached to the vehicle
        existing_sensors = self._find_attached_sensors()

        # Set up additional monitoring sensors (collision and lane invasion)
        self.collision_sensor = CollisionSensor(self.player, self.hud)
        self.lane_invasion_sensor = LaneInvasionSensor(self.player, self.hud)

        # Try to find existing GNSS and IMU sensors
        if "gnss" in existing_sensors:
            self.gnss_sensor = GnssSensor(self.player, existing_sensors["gnss"])
            print(f"Found existing GNSS sensor")
        else:
            self.gnss_sensor = GnssSensor(self.player)

        if "imu" in existing_sensors:
            self.imu_sensor = IMUSensor(self.player, existing_sensors["imu"])
            print(f"Found existing IMU sensor")
        else:
            self.imu_sensor = IMUSensor(self.player)

        # Set up camera manager (creates visualization camera)
        self.camera_manager = CameraManager(self.player, self.hud, self._gamma)
        self.camera_manager.transform_index = cam_pos_index
        self.camera_manager.set_sensor(cam_index, notify=False)

        actor_type = get_actor_display_name(self.player)
        self.hud.notification(f"Monitoring: {actor_type}")
    ...
    def _find_attached_sensors(self):
        """Find sensors already attached to the vehicle."""
        sensors = {}
        all_actors = self.world.get_actors()

        for actor in all_actors:
            # Check if this actor is attached to our vehicle
            try:
                if hasattr(actor, "parent") and actor.parent is not None:
                    if actor.parent.id == self.player.id:
                        actor_type = actor.type_id
                        if "gnss" in actor_type:
                            sensors["gnss"] = actor
                            print(f"Found attached GNSS sensor: {actor_type}")
                        elif "imu" in actor_type:
                            sensors["imu"] = actor
                            print(f"Found attached IMU sensor: {actor_type}")
                        elif "lidar" in actor_type or "ray_cast" in actor_type:
                            # We could track LiDARs if needed
                            print(f"Found attached LiDAR sensor: {actor_type}")
            except Exception as e:
                # Some actors might not have parent attribute
                pass
    ...
    def destroy(self):
        if self.radar_sensor is not None:
            self.toggle_radar()
        # Only destroy sensors we created for monitoring
        sensors_to_destroy = [
        ...
        # Only destroy GNSS/IMU if we created them (not using existing ones)
        existing_sensors = self._find_attached_sensors()
        if (
            "gnss" not in existing_sensors
            and hasattr(self, "gnss_sensor")
            and hasattr(self.gnss_sensor, "sensor")
        ):
            sensors_to_destroy.append(self.gnss_sensor.sensor)
        if (
            "imu" not in existing_sensors
            and hasattr(self, "imu_sensor")
            and hasattr(self.imu_sensor, "sensor")
        ):
        ...
        # Don't destroy the vehicle - it's managed by vehicle_spawner
        # if self.player is not None:
        #     self.player.destroy()
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        if isinstance(world.player, carla.Vehicle):
            ...
            try:
                ...
                # Try to get current autopilot state if possible
                self._autopilot_enabled = world.player.get_control().manual_gear_shift

            try:
                world.player.set_light_state(self._lights)
            except RuntimeError as e:
                print(f"Warning: Failed to set light state: {e}")
            ...
            if isinstance(self._control, carla.VehicleControl):
                ...
                if (
                    ...
                    world.player.set_light_state(
                        carla.VehicleLightState(current_lights)
                    )
                # Apply control
                if not self._ackermann_enabled:
...
class GnssSensor(object):
    def __init__(self, parent_actor, existing_sensor=None):
        self.sensor = None
        self._parent = parent_actor
        self.lat = 0.0
        self.lon = 0.0
...
def game_loop(args):
    ...
    try:
        ...
        # Removed autopilot warning since vehicle is controlled by spawner
        ...
        # Start with autopilot disabled - vehicle is controlled by spawner
...
def main():
    argparser = argparse.ArgumentParser(
        description="CARLA Vehicle Monitor - Monitor vehicles spawned by vehicle_spawner"
    )
    ...
    argparser.add_argument(
        ...
        default=2000,
        type=int,
        help="TCP port to listen to (default: 2000 to match vehicle_spawner)",
    )
    argparser.add_argument(
        "--res",
    ...
    argparser.add_argument(
        ...
        metavar="NAME",
        default="ego_vehicle",
        help='vehicle role name to monitor (default: "ego_vehicle")',
    )
    argparser.add_argument(
        "--gamma",
        default=1.0,
    ...
    argparser.add_argument(
        "--filter", metavar="PATTERN", default="vehicle.*", help=argparse.SUPPRESS
    )  # Hidden, kept for compatibility
    argparser.add_argument(
        "--generation", metavar="G", default="All", help=argparse.SUPPRESS
    )  # Hidden, kept for compatibility
